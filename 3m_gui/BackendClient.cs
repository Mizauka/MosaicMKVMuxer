...
/// <summary>
/// BackendClient — 与 Rust m3_core 后端 HTTP API 通信。
/// 启动时自动 spawn 后端进程，通过 HTTP + SSE 调用所有功能。
/// </summary>
public sealed class BackendClient : IDisposable
{
    ...
    public bool IsRunning => _started && _backendProcess is { HasExited: false };
    ...
    public async Task<bool> StartAsync(CancellationToken ct = default)
    {
        ...
        // 启动 stderr 异步读取（避免缓冲区满导致阻塞）
        _ = ReadStderrAsync(_backendProcess);

        // 等待就绪信号（循环读取直到出现 "M3_CORE_READY"），10 秒超时
        try
        {
    ...
    {
        // 1. 与 GUI 同目录（CopyRustBackend 构建目标会复制到这里）
        ...
        // 2. 从 BaseDirectory 向上搜索项目根目录中的 3m_core
        string[] configs = ["release", "debug"];
        try
        {
            var dir = new DirectoryInfo(AppContext.BaseDirectory);
            // 向上最多 8 层寻找 3m_core 目录
            for (int i = 0; i < 8; i++)
            {
                dir = dir.Parent;
                if (dir == null) break;
    ...
    {
        if (_port == 0) throw new InvalidOperationException("后端端口未分配，请先调用 StartAsync");
    ...
    public async Task<string?> GetVersionAsync(CancellationToken ct = default)
    {
        ...
        var resp = await _http.GetFromJsonAsync<JsonElement>(Url("/api/version"), ct);
        return resp.TryGetProperty("version", out var v) ? v.GetString() : null;
    }

    private static HttpContent EmptyBody() => new ByteArrayContent([]);
    ...
    public async Task<string?> DetectFFmpegAsync(CancellationToken ct = default)
    {
        ...
        {
            Logger.Error(ex, "DetectFFmpegAsync");
            throw;
        }
    }

    public async Task<string?> Detect7zAsync(CancellationToken ct = default)
    {
        try
        {
            Logger.Info("POST /api/detect/7z");
            var resp = await _http.PostAsync(Url("/api/detect/7z"), EmptyBody(), ct);
            resp.EnsureSuccessStatusCode();
            var result = await resp.Content.ReadFromJsonAsync<DetectResult>(ct);
            Logger.Info($"7-Zip 检测结果: {result?.Path ?? "(null)"}");
            return result?.Path;
        }
        catch (Exception ex)
        {
            Logger.Error(ex, "Detect7zAsync");
            throw;
        }
    }
    ...
    public async Task<List<EpisodeInfo>> ScanWorkspaceAsync(string dir, CancellationToken ct = default)
    {
        ...
        {
            Logger.Error(ex, "ScanWorkspaceAsync");
            throw;
        }
    }
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        var req = new { ffmpeg = ffmpegPath, input = inputDir, output = outputDir };
        ...
        {
            Logger.Error(ex, "StartMuxAsync 请求失败");
            throw;
        }
